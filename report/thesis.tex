...
\fixed{Add one sentence to summarize the results.}
...
\fixed{You should write a paragraph with an example on how the service
...
\newstuff{moved text from implementation}
...
The SL adaptation is  performed every \textcolor{red}{???} job. The average performance of the last ten jobs are then sent to the RM and the SL adaptation is made as described in the previous section. See the sequence diagram  \ref{fig:sdiag} for the application loop.
...
\fixed{Write the equations for the resource adaptation. Take them from the
...
\fixed{Can you comment a bit on what is the difference between CPUShares
...
\fixed{Can you add a couple of information about the processing? How are
...
\fixed{Add a short introductory sentence above. This sentence should
...
\fixed{Will you present test with both the cameras or only with one of
...
\begin{framed}
	...
	\end{flushleft}	
\end{framed}
\fixed{Why did you need to add the happy field? That's a difference
...
In ~\ref{sec:systemdandcgroup} we introduced CPUShares, which assigns CPU-resources to an application proportionally to the amount of shares received 
...
\martina{Look at the next paragraph. You will need to add something like
  this for every experiment. You must comment the graphs.}
...
therefore they help the infrastructure up to different extents. For a summary
...
\martina{Comment here: are the virtual platform assignments respecting
  the theory? The application that should help more with the SL is helping
  more or not? You have to show that this experiment is significant in terms
  of what you learn about the system.} 
...
\martina{Include in the figure for the virtual platform 0.2 in the
  axis, to make sure that you actually see the signals. Are they
  really at 0.25 or it is simply assumed? Another comment that you can
  make here is the role of $\epsilon$. Do you see that the applications 3
  and 4 are way slower in adjusting their service level? What does that
  mean? You should add something about it.}
