...
\end{lstlisting}
\fixed{Instead of the nested else if, wouldn't it be better to have a
  switch in the code above?}
\newstuff{Maybe but this is part of the systemd source code, not written by us}
...
\fixed{Check that the next sentence describes what you actually did.
  ...
  version.}
  \newstuff{We tried rewriting it again since your changes didn't really match what we where trying to say.}
...
\fixed{Is it the same example of Figure~\ref{fig:Piechart}? If so,
...
\fixed{Is the application always waiting to receive the performance
...
for app2 becomes different since the $a_{cpu}$ is twice that of the other apps resulting in a half as big SL compared to what it would have been otherwise. The VP resets after a new app has been started as it should. The SL converges slower than app1/2 because of the lower epsilon.
...
\martina{When you comment it, say that the service level shown in the
  figure has no sense for application 1 and 3, since they don't adapt
  the computation that they do but it is simply a value. Alternatively,
  exclude them from the picture. This second alternative will also show
  better the adaptation of the other service levels.}
...
\fixed{What do the results show? Beside the fact that it is usable?
