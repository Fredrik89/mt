...
Once an event has been popped from the prioritized queue the callback function \emph{static int manager\_dispatch\_gtrm\_fd(...)} is called. The instructive parts of the function are shown below.
...
\end{verbatim}
Before the loop actually start to run, we create and initiate a struct that contain the necessary data for the resource manager, \emph{gtrm\_t}.
...
\begin{verbatim}
  ...
  while (m->exit_code == MANAGER_RUNNING) {
    ...
\end{verbatim}
Inside the loop, an if-statement makes sure we do not run the resource manager if it is not needed. 
\begin{verbatim}
...
\end{verbatim}
Seconod we compute the virtual platforms and update the number of iterations.
\begin{framed}
		\begin{flushleft}
...
\end{verbatim}
If we exit the loop we have to deallocate the \emph{gtrm\_t} struct to avoid memory leakage.
\begin{verbatim}
  free(gtrm_t);
  return m->exit_code;
}
...
\subsubsection{double gtrm\_lib\_get\_performance\_number(\_application\_h* a, int job\_type)}\subsubsection{static int manager\_setup\_gtrm(Manager *m)}
Used to setup the socket, event source and file descriptor for the resource manager. Called from manager\_startup and manager\_reload.
\begin{itemize}
\item m: Reference to the manager.
\item Return value: Zero if function ran correctly, otherwise it is set to the corresponding error number.
