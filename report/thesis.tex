...
\fixed{If you have not made an example of CPUShares before (see the
...
to create a service file in the same folder. When the service is started a folder named \texttt{gtrm.slice} will be created in the corresponding cgroup-controller-folder to represent the slice. The controller is a combination of the CPU-and CPUAccounting, \texttt{cpu,cpuacct}. The applications run in this slice will be represented as a folder in the slice-folder.
...
The remaining application are located on the static slice. On the camera (and all other systemd systems) this is called the system slice, and its unit file is located in the folder \texttt{/usr/lib/systemd/system}.
...
is notified via the \texttt{send\_performance()} call. Receiving the performance multiplier is done via a non-blocking socket so the application does not have to wait for the resource manager to send it.
