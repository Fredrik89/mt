...
algorithm. CPUShares are used here in place of bandwidth, which is independent on the amount of CPU cores for instance. This means that the resource manager does not have to compensate nor keep track of the amount of CPU-cores of the system, as opposed to the bandwidth implementation.
...
The bit-rate is the amount of data that flows through the application, measured in kbit/s. This means that a higher bit-rate gives better image quality, since we have more data per frame, but at the cost of higher computation time. In the event that the frame rate drops because of limited CPU-resources, the bit-rate can be reduced which allows a higher frame rate. The bit-rate controller is a controller which aims to keep a certain bit-rate, independent on how the imagery looks. If the scenery becomes more complex this will increase the bit-rate. To cope with this the image quality is reduce by the controller which will decrease the bit-rate to desired level. The image quality is adjusted by changing the amount of compression.
...
\subsection{adsf}
Start four test-applications at approximately the same time:
