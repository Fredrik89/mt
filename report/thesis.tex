...
Before the loop actually start to run, we create and initiate a struct that contain the necessary data for the resource manager, \emph{gtrm\_t}. Inside the loop, the if-statement makes sure we do not run the resource manager if it is not needed. The first step is to update the number of running applications. Then we compute the virtual platforms 
