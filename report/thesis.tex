...
Overall the plots of the virtual platforms and the service level look 
good, depite extremely noisy performance measurements. The matching functions
are however signals with zero mean, which is the main objective of the
...
performance up. However when loading the system slice with a CPU hunger
application, no major changes in performance were shown.
Another explanation could be the delay in the sockets communication,
since delays tend to make systems oscillate. Also, it has to be noticed
that the time between actions from the GTRM is irregular. The effect of this aperiodicity on the system has not been taken into account.
