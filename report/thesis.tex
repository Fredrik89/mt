...
Both gtrm.slice and system.slice have been given $CPUShares = 1024$ meaning that the cpu resources are split 50/50 between the two slices. The test applications takes 10 arguments when started, described in the box below, that will be varied between different test. By varying the weight parameter from 0 to 1 the applications can be set somewhere between the extremes of only adapting through resource allocation or only adapt through SL adaptation.
...
\section{Results}
\subsection{Test 1}
The first test uses the applications specified in table ~\ref{tab:test1apps}.

\begin{table}[h]
\centering
\begin{tabular}{ccccccc}
\hline
 & $a^\mathsf{cpu}_i$ & $b^\mathsf{cpu}_i$
 & $a^\mathsf{mem}_i$ & $b^\mathsf{mem}_i$
 & $\epsilon_i$ & $\lambda_i$ \\
\hline
$\mathsf{app}_1$ & 0 & 1000 & 0 & 0 & - & 0.5 \\
$\mathsf{app}_2$ & 1000 & 0 & 0 & 0 & 0.1 & 0.5 \\
$\mathsf{app}_3$ & 0 & 1000 & 0 & 0 & - & 0.2 \\
$\mathsf{app}_4$ & 1000 & 0 & 0 & 0 & 0.1 & 0.2 \\
$\mathsf{app}_5$ & 100 & 0 & 0 & 0 & 0.01 & 0.8 \\
\hline
\end{tabular}
\caption{Applications used in Test 1.}
\label{tab:test1apps}
\end{table}
...
\subsection{Test 2}
The second test uses applications specified in table ~\ref{tab:test2apps}.
...
\hline
\end{tabular}
\caption{Applications used in Test 2.}
\label{tab:test2apps}
\end{table}
