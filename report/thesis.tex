...
The Axis M1033 is a small camera as can be seen in figure ~\ref{fig:M1033}, connected to the network either wired or wireless. 
...
The Axis P3367~\cite{p3367} is a fixed dome network camera, see Figure ~\ref{fig:P3367} and ~\ref{fig:P3367case}, which is capable of 
multiple H.264 streams as well as Motion JPEG streams. It supports 
various frame rates and resolutions of up to 5 Megapixel at 12 frames per 
...
are mapped into some parameter changes, that in turn affects the resource
...
Note that the compression is done on dedicated hardware and not by the CPU being controlled by the \emph{GTRM}, hence the decrease in execution time when lowering
...
in Equations~\ref{eq:simple_sl_rec} 
and~\ref{eq:not_simple_sl_rec}. The resulting service level is used to set the size of each frame, where a larger frame has higher image quality.
...
\sfrac{1}{3} of the total resources while the two \emph{GTRM} services are given 
...
\section{manager.c}
Note that this file is a part of \texttt{systemd} and only what was added or changed to this file is included in the following code-snippets. For the rest of the code in this file, please refer to the open-source project~\cite{sysd1}.
...
static int manager_dispatch_gtrm_fd(sd_event_source *source, int fd, uint32_t revents, void *userdata) {
	...
	fprintf(stderr,"Read: %d messages ",counter);
	m->update_gtrm = true;
	return 0;
}
...
int gtrm_compute_virtual_platforms(Hashmap *apps, gtrm_t *gtrm_t) {
	...
	HASHMAP_FOREACH(a, apps, i) {
		sumlambdafi += (a->weight)* a->performance;
	}
	...
	HASHMAP_FOREACH(a, apps, i) {
		...
		float tmp = vp_old - epsilon * ( a->weight * a->performance - (sumlambdafi * vp_old)); // computed as sum to one
...
void gtrm_char2gtrmstruct(char* str, rm_app_t *re){	
	...
	pch = strtok (str,"x");
	re->tid = (uint)atoi(pch);

	pch = strtok (NULL,"x");
	re->performance = atof(pch);
	
	pch = strtok (NULL,"x");
	re->happy = (uint)atoi(pch);
...
void gtrm_print_struct(rm_app_t *rm ){
	printf("\ntid:%d\nvp:%f\nvp_old:%f\nperformance:%f\nweight:%f\nhappy:%d\n",
	rm->tid,rm->vp,rm->vp_old,rm->performance,rm->weight,rm->happy);
}

int gtrm_send_performance_multiplier(double pm,int fd, struct sockaddr *sa){
	int n;
	socklen_t tolen;
	tolen = (socklen_t) sizeof(struct sockaddr_un);
	char buf[512];//2do no magic numbers
	memset(buf,'\0',512);
	sprintf(buf,"%f",pm);
	n=sendto(fd,buf,512,0,(struct sockaddr *) sa,tolen);
	return 0;
}
...
double gtrm_get_epsilon(unsigned int iterations, unsigned int offset, double c1, double c2) {
	...
	if(iterations>0)														
		value = value * (double)(current_time - time_since_start)/1000000000.0;
	else 
	{
		time_since_start = current_time;
		return 1;
...
void gtrm_update_performance_multipliers(int gtrm_fd, Hashmap *gtrm_apps){
	...
	rm_app_t *a;
	double pm;
	HASHMAP_FOREACH(a, gtrm_apps, i) {				
		if(a->vp_old != 0){		
			pm = (1+a->performance)*(a->vp/a->vp_old);
			gtrm_send_performance_multiplier(pm,gtrm_fd,a->sa);			
		}					
	}
}
...
void gtrm_write_log(Hashmap *gtrm_apps, unsigned int num_applications) {
	if (num_applications > 0) {
		...
		current_time = (int64_t) time_info.tv_sec*1000000000 
		+ (int64_t) time_info.tv_nsec;
		FILE* logfile = fopen("/root/gtrm.log", "a+");
		...
		HASHMAP_FOREACH(a, gtrm_apps, i) {
			...
			current_time, a->tid, a->vp,
			a->performance, a->weight);
		}
	fclose(logfile);
	}
...
void gtrm_lib_send_performance(_application_h *h,double performance){
	...
	if(n<0){
			perror("sendto");
			fprintf(stderr,"could not send performance");
	}
}
...
void
setup_sl_adapt (float service_level_, double epsilon_, double weight_,
    double deadline_seconds_)
{
	...
	sock_name = (char *) calloc (1, 200); //2do add free
	sock_path = (char *) calloc (1, 200); //2do add free
	sprintf (sock_path, "/mnt/flash/fd");
	int temp_pid = (int) getpid ();

	myself = calloc (1, sizeof (_application_h));
	sprintf (sock_name, "%s/%d", sock_path, temp_pid);
	gtrm_lib_setup_socket ((char *) sock_name);
	uint64_t deadline = (unsigned int) ((double) 1000000000 * deadline_seconds);
	uint64_t ert[1] = { deadline };
	gtrm_lib_set (myself, 1, ert);
	myself->weight = weight_;
	myself->application_id = getpid ();
}
...
{
	id = 0;
	gtrm_lib_signalend (myself, id);
	double performance;

	int type = 0;
	id = gtrm_lib_signalstart (myself, type);
	...
	if (service_level != service_level) // avoid nans
		...
		int64_t current_time;
		clock_gettime (CLOCK_REALTIME, &time_info);
		current_time = (int64_t) time_info.tv_sec * 1000000000 + (int64_t) time_info.tv_nsec;

		char name[200];
		sprintf (name, "/mnt/flash/logs/%u.log", myself->application_id);
		FILE *logfile = fopen (name, "a+");

		if (logfile == NULL)
			perror ("could not open file");
		assert (logfile != NULL);
		fprintf (logfile, "%lld, %f, %f, %lld,%u\n",
